...
const initializeDefaultInstitutions = () => {
  const institutions = [
    ...
    },
    {
      ...
      port: 3002,
      privateKey:
        "b2c3d4e5f6789abcdef1234567890abcdef1234567890abcdef1234567890abc1",
    },
    {
      ...
      port: 3003,
      privateKey:
        "c3d4e5f6789abcdef1234567890abcdef1234567890abcdef1234567890abc12",
    },
  ];

  // Find and configure this node's institution
  const institutionConfig = institutions.find(
    (inst) => inst.port === parseInt(port),
  );
  ...
  if (institutionConfig) {
    ...
    certificateBlockchain.institutionRegistry.registerInstitution(
      nodeInstitution,
    );
    certificateBlockchain.addAuthorizedValidator(publicKey);

    Logger.info(
      `Node initialized as: ${nodeInstitution.name} (${nodeInstitution.type})`,
    );
...
app.post("/register-and-broadcast-node", async (req, res) => {
  try {
    ...
    const regNodesPromises = certificateBlockchain.networkNodes.map(
      (networkNodeUrl) => {
        return rp({
          ...
          Logger.warn(
            `Failed to register with ${networkNodeUrl}:`,
            err.message,
          ),
        );
      },
    ...
    await rp({
      ...
      body: {
        allNetworkNodes: [
          ...certificateBlockchain.networkNodes,
          certificateBlockchain.currentNodeUrl,
        ],
      },
      json: true,
      timeout: 5000,
...
app.post("/register-node", (req, res) => {
  ...
  if (
    !certificateBlockchain.networkNodes.includes(newNodeUrl) &&
...
app.post("/register-nodes-bulk", (req, res) => {
  ...
  allNetworkNodes.forEach((networkNodeUrl) => {
    if (
      !certificateBlockchain.networkNodes.includes(networkNodeUrl) &&
...
app.post("/certificates", async (req, res) => {
  try {
    ...
    if (!nodeInstitution) {
      return res
        .status(400)
        .json({ error: "Node is not configured as an authorized institution" });
    }

    if (!recipientWalletAddress) {
      return res
        .status(400)
        .json({ error: "Recipient wallet address is required" });
    }
    ...
    const broadcastPromises = certificateBlockchain.networkNodes.map(
      (networkNodeUrl) => {
        return rp({
          ...
          Logger.log(`Failed to broadcast to ${networkNodeUrl}:`, err.message),
        );
      },
    );
...
app.post("/certificates/:id/verify", async (req, res) => {
  try {
    ...
    const verificationResult =
      certificateBlockchain.verifyCertificate(certificateId);
      ...
      const broadcastPromises = certificateBlockchain.networkNodes.map(
        (networkNodeUrl) => {
          return rp({
            ...
            Logger.log(
              `Failed to broadcast verification to ${networkNodeUrl}:`,
              err.message,
            ),
          );
        },
    ...
    const broadcastPromises = certificateBlockchain.networkNodes.map(
      (networkNodeUrl) => {
        return rp({
          ...
          Logger.log(
            `Failed to broadcast revocation to ${networkNodeUrl}:`,
            err.message,
          ),
        );
      },
    ...
    const requestPromises = certificateBlockchain.networkNodes.map(
      (networkNodeUrl) => {
        return rp({
          ...
          Logger.log(
            `Failed to broadcast block to ${networkNodeUrl}:`,
            err.message,
          ),
        );
      },
...
app.post("/receive-new-block", (req, res) => {
  try {
    ...
    const correctIndex =
      (lastBlock.index || certificateBlockchain.chain.length - 1) + 1 ===
      (newBlock.index || certificateBlockchain.chain.length);
    ...
    for (let nodeUrl of nodeUrls) {
      if (nodeUrl !== certificateBlockchain.currentNodeUrl) {
        await rp({
          ...
          Logger.log(`Failed to initialize ${nodeUrl}:`, err.message),
        );
      }
    }

    // Update local network nodes
    certificateBlockchain.networkNodes = nodeUrls.filter(
      (url) => url !== certificateBlockchain.currentNodeUrl,
    );
...
app.post("/wallets", (req, res) => {
  try {
    ...
    Logger.log(
      `Created new wallet: ${walletResponse.id} (${walletResponse.label})`,
    );
      ...
      const demoWallets = [
        {
          ...
          publicKey:
            "04a1b2c3d4e5f6789abcdef1234567890abcdef1234567890abcdef1234567890ab1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef",
          certificateCount: 0,
          type: "INDIVIDUAL",
        ...
        },
        {
          ...
          publicKey:
            "04b2c3d4e5f6789abcdef1234567890abcdef1234567890abcdef1234567890abc1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef1",
          certificateCount: 0,
          type: "INDIVIDUAL",
    ...
    try {
      ...
      allWallets.forEach((wallet) => {
        wallet.certificateCount = allCertificates.filter((cert) => {
          ...
          if (
            cert.metadata &&
            cert.metadata.recipientWalletAddress === wallet.publicKey
          ) {
            return true;
          }
      ...
      Logger.log(
        `Warning: Could not load certificates for wallet counts: ${certError.message}`,
      );
...
app.get("/wallets/:publicKey", (req, res) => {
  try {
    ...
    if (
      !walletInfo &&
      nodeInstitution &&
      nodeInstitution.publicKey === publicKey
    ) {
      walletInfo = {
        id: "institution-wallet",
        label: `${nodeInstitution.name} Wallet`,
    ...
    try {
      ...
      if (typeof certificateBlockchain.getWalletCertificates === "function") {
        walletCertificates =
          certificateBlockchain.getWalletCertificates(publicKey);
      }
      ...
      Logger.log(
        `Warning: Could not load certificates for wallet ${publicKey}: ${certError.message}`,
      );
      // Fallback to manual search if blockchain methods fail
      try {
        const allCertificates =
          certificateBlockchain.getAllCertificates() || [];
        walletCertificates = allCertificates.filter((cert) => {
          ...
          if (
            cert.metadata &&
            cert.metadata.recipientWalletAddress === publicKey
          ) {
            return true;
          }
        ...
        Logger.log(
          `Warning: Fallback certificate loading also failed for wallet ${publicKey}: ${fallbackError.message}`,
        );
      }
    }
    ...
    try {
      certificateBlockchain.chain.forEach((block) => {
        if (block.transactions) {
          block.transactions.forEach((tx) => {
            if (
              tx &&
              (tx.fromAddress === publicKey || tx.toAddress === publicKey)
            ) {
      ...
      Logger.log(
        `Warning: Could not load transactions for wallet ${publicKey}: ${txError.message}`,
      );
    }
    ...
    const walletResponse = {
      ...
      isInstitution:
        publicKey === (nodeInstitution ? nodeInstitution.publicKey : null),
    };
    ...
    Logger.log(
      `Error fetching wallet ${req.params.publicKey}: ${error.message}`,
    );
    res.status(500).json({
      success: false,
      message: error.message,
...
app.get("/wallets/:publicKey/certificates", (req, res) => {
  try {
    ...
    Logger.log(
      `Error fetching certificates for wallet ${req.params.publicKey}: ${error.message}`,
    );
    res.status(500).json({
      success: false,
      message: error.message,
...
app.get("/wallets/:publicKey/transactions", (req, res) => {
  try {
    ...
    const pendingTransactions = mempool
      .getAllTransactions()
      .filter(
        (tx) => tx.fromAddress === publicKey || tx.toAddress === publicKey,
      )
      .map((tx) => ({
        ...tx,
        status: "PENDING",
    ...
    const allTransactions = [...transactions, ...pendingTransactions].sort(
      (a, b) => new Date(b.timestamp) - new Date(a.timestamp),
    );
    ...
    Logger.log(
      `Error fetching transactions for wallet ${req.params.publicKey}: ${error.message}`,
    );
    res.status(500).json({
      success: false,
      message: error.message,
...
app.listen(port, () => {
  ...
  if (nodeInstitution) {
    Logger.log(
      `Institution: ${nodeInstitution.name} (${nodeInstitution.type})`,
    );
  }
});
...
const performAutoProcessing = async () => {
  try {
    ...
    if (
      !nodeInstitution ||
      certificateBlockchain.getPendingTransactions().length === 0
    ) {
      return;
    }

    Logger.log(
      `Auto-processing: Found ${certificateBlockchain.getPendingTransactions().length} pending certificate transactions`,
    );
    ...
    Logger.log(
      `Auto-processing: Certificate block processed successfully - ${newBlock.hash}`,
    );
    ...
    const requestPromises = certificateBlockchain.networkNodes.map(
      (networkNodeUrl) => {
        return rp({
          ...
          Logger.log(
            `Auto-processing: Failed to broadcast block to ${networkNodeUrl}:`,
            err.message,
          ),
        );
      },
...
const startAutoProcessing = () => {
  ...
  if (autoProcessingEnabled && nodeInstitution) {
    Logger.log(
      `Auto-processing started: Processing every ${PROCESSING_INTERVAL_MS / 1000} seconds`,
    );

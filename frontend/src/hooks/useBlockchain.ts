...
export const useNetwork = () => {
  ...
  const fetchNetworkData = useCallback(async () => {
    ...
    try {
      ...
      setError(
        error instanceof Error ? error.message : "Failed to fetch network data",
      );
    } finally {
      setLoading(false);
    }
  ...
  const initializeNetwork = useCallback(async () => {
    ...
    try {
      ...
      setError(
        error instanceof Error ? error.message : "Failed to initialize network",
      );
      return false;
    } finally {
      setLoading(false);
  ...
  const checkConnection = useCallback(async (isAutomatic = false) => {
    ...
    try {
      ...
      if (
        ...
        friendlyError =
          "Connection timeout. The backend server may be overloaded.";
      } else if (errorMessage.includes("404")) {
        friendlyError =
          "Backend API not found. Please check the server configuration.";
...
export const useAutoRefresh = (intervalMs: number = 60000) => {
  ...
  const refreshAll = useCallback(async () => {
    try {
      await Promise.all([blockchain.refresh(), pendingTransactions.refresh()]);
